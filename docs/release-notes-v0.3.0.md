...
- Downstream HTTP/2 support through Axum/Hyper's HTTP/2 stack.
...
- Origin protocol config for `auto`, `http1`, and `http2` preferences.
- Optional HTTP/2 prior knowledge for trusted origins.
- Performance config for max buffered response size, streaming unstoreable bodies, observer shard planning, async disk write policy, and origin connection pool tuning.
...
- HTTP/2 flow-control settings are parsed for config compatibility; deeper per-connection tuning remains a follow-up.
