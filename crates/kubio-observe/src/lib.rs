...
struct ObserverInner {
    ...
    alt_svc: AltSvcCounts,
    http3_server: Http3ServerCounts,
    upstream_http3: UpstreamHttp3Counts,
}
...
impl AltSvcCounts {
    fn increment(&mut self, outcome: AltSvcOutcome, reason: AltSvcReason) {
        if outcome == AltSvcOutcome::Advertised {
            self.advertised += 1;
            return;
        }
...
impl Http3ServerCounts {
    fn increment(&mut self, event: Http3ServerEvent) {
        match event {
...
impl UpstreamHttp3Counts {
    fn increment(&mut self, event: UpstreamHttp3Event) {
        match event {
